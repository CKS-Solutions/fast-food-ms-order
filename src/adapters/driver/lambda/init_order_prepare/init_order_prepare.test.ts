import { handler } from './init_order_prepare';
...
describe('SetOrderInPreparation HTTP Handler', () => {
  ...
  it('should update order status to InPreparation successfully', async () => {
    const event = makeEvent('order-123');

    mockExecute.mockResolvedValue({
      message: 'Order order-123 status updated to InPreparation successfully',
    });
  ...
  it('should return HTTP error response when use case throws HTTPError', async () => {
    ...
    mockExecute.mockRejectedValue(
      new HTTPBadRequest('Order cannot be moved to InPreparation'),
    );
